...
if ($devices.Count -eq 0) {
  Write-Host "기기 미연결 — USB 또는 무선 디버깅으로 연결 후 수동으로 실행:"
  Write-Host "  adb reverse tcp:8081 tcp:8081"
  Write-Host "  adb reverse tcp:8082 tcp:8082"
} elseif ($devices.Count -eq 1) {
  & $adb reverse tcp:8082 tcp:8082
  & $adb reverse tcp:8081 tcp:8081
  Write-Host "adb reverse 8081/8082 OK"
} else {
  Write-Host "기기 여러 대 감지 — -s 옵션으로 지정 필요:"
  & $adb devices
  Write-Host "  adb -s <serial> reverse tcp:8081 tcp:8081"
  Write-Host "  adb -s <serial> reverse tcp:8082 tcp:8082"
}
```

**무선 연결 방법** (1회 페어링 후 매번 connect):
1. 폰: 개발자 옵션 → 무선 디버깅 → 페어링 코드로 기기 페어링
2. `adb pair <IP>:<페어링포트> <6자리코드>`
3. `adb connect <IP>:<메인포트>` (무선 디버깅 화면 상단 IP:포트)
