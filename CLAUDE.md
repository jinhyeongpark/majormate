...
기능 구현 시 `/harness` 커맨드로 step 파일을 설계한 뒤 `python3 scripts/execute.py {phase-name}`으로 자동 실행한다.

### 서버 태스크 TDD 순서 (필수)

Service 레이어 구현이 포함된 **모든** 서버 태스크에서 아래 순서를 반드시 따른다:

1. **test agent 호출** — 메서드 시그니처 + 비즈니스 룰을 전달해 실패 테스트 작성
2. **server agent에 RED 확인 지시** — `./gradlew test --tests "<TestClass>"` 실행 → 실패 확인
3. **server agent 구현** — 테스트 파일을 스펙으로 삼아 구현 → GREEN
4. **GREEN 재확인** — `./gradlew test` 전체 재실행

**예외**: DB 마이그레이션, 컨트롤러 라우팅, DTO 정의만 변경하는 태스크는 test agent 호출 생략 가능.
...
- `docs/PRD.md` — 전체 기능 명세, UX 플로우, 수익화 모델
- `phases/index.json` — 전체 phase 목록 및 진행 상태
- `phases/{N}/spec.md` — 각 phase 기획 명세 및 태스크 목록
